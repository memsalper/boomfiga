...
function Item({title, poster, navigation, sinif, id, trackId, rate, order, countProcess}) {
    let tur = 5;
    return (
        <TouchableOpacity style={styles.item} onPress={() => {
            ...
            <Block flex space="between" style={styles.cardDescription}>
                ...
                <Block row space="between">
                    ...
                    <Text size={12} muted={false} color={argonTheme.COLORS.GRADIENT_END} bold>Dinlenme Sayısı : {countProcess}</Text>
                </Block>
            </Block>
        </TouchableOpacity>
    );
...
export class MuzikListesi extends Component {
    constructor(props) {
        super(props);
        this.state = {
                ...
                {!topList ?
                    <FlatList style={{width: '100%'}}
                              ...
                              renderItem={({item}) => <Item
                                  title={item.name ? item.artists[0].name + ' - ' + item.name : item.name}
                                  poster={item.album.images[1].url ? item.album.images[1].url : item.cover.url}
                                  navigation={navigation} sinif={this} id={item.album.id} trackId={item.id}
                                  order={item.key} rate={item.rate} countProcess={0}/>}
