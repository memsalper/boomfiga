    ...
    return (
        <TouchableOpacity style={styles.item} onPress={() => {
            ...
            <View style={{justifyContent: 'center', alignItems: 'center', paddingLeft: 5}}>
                <Text size={20} color={theme.COLORS.SIYAH} bold>{order}</Text>
            </View>
...
export class FilmListesi extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            <View style={{flex: 1, width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 5}}>
                ...
                {this.state.isChoosen ? null :
                    <FlatList style={{width: '100%'}}
                              data={veri}
                              renderItem={({item}) => <Item title={item.title} poster={item.poster_path}
                                                            navigation={navigation} sinif={this} id={item.id}
                                                            order={item.key} rate={item.rate}
